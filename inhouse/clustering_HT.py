...
data = np.load("data/embeddings_TTHT.npz", allow_pickle=True)
...
reducer_15d = umap.UMAP(
    n_components=30,
    n_neighbors=50,  # adjust: small for local, larger for global
    min_dist=0.0,
    metric="cosine",
    random_state=42,
)
...
cluster_dict.pop(-1, None)  # Remove noise label (-1) if present
